...
class Image_Processor:
    ...
    def apply_blur_gaussian(self):
        ...
        self.img = Image.fromarray(gaussian_filter(arr, sigma=(1.7, 1.5, 0)).astype(np.uint8))
    ...
    def apply_vignette(self):
        ...
        pixel_dist_from_center = np.sqrt((x - center_x_position) ** 2 + (y - center_y_position) ** 2)
    ...
    def apply_film_grain(self):
        ...
        sigma = 20
        noise_layer = (np.array(Image.effect_noise((width, height), sigma))[..., np.newaxis] - 128) * 0.15
        # print(str(noise_layer.max()))

        self.img = Image.fromarray(np.clip((arr + noise_layer), 0, 255).astype(np.uint8))
